...
class CVRPModel(nn.Module):
    ...
    def __init__(self, inference, **model_params):
        ...
        if inference:
            ...
            self.decoder = nn.ModuleList([CVRP_Decoder(**model_params) for _ in range(model_params['num_dis'])])
    ...
    def pre_forward(self, reset_state):
        ...
        if self.inference:
            ...
            sub_batch_size = reset_state.node_demand.size(0) // self.model_params['num_dis']
            ...
            if self.inference:
                ...
                for i in range(self.model_params['num_dis']):
                    prob = self.decoder[i](encoded_last_node[i * sub_batch_size: (i + 1) * sub_batch_size],
                                           load=state.load[i * sub_batch_size: (i + 1) * sub_batch_size],
